...
import { ChevronLeft, ChevronRight, CheckCircle, Upload, Loader2 } from "lucide-react";
...
export default function EditListingPage() {
  ...
  const { data: propertyResponse, isLoading } = useProperty(id || "");
  ...
  const onSubmit = (data: PropertyFormData) => {
    ...
    updateProperty.mutate({ id: property._id, formData }, {
      ...
      >
          ...
          <Card className="animate-fade-in">
            ...
            <CardContent>
              {property.images && property.images.length > 0 && (
                <div className="mb-6">
                  <Label className="mb-2 block">Existing Images ({property.images.length}/5)</Label>
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {property.images.map((img, i) => (
                      <img key={i} src={img} alt={`Property image ${i + 1}`} className="h-24 w-24 object-cover rounded-md flex-shrink-0 border" />
                    ))}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">New images will be appended to the existing ones. (Maximum 5 total)</p>
                </div>
              )}
              ...
              <ImageUploadInput
                ...
                maxImages={Math.max(0, 5 - (property.images?.length || 0))}
              />
            </CardContent>
          </Card>
        )}
        ...
        {updateProperty.isError && (
          <div className="mt-4 p-3 rounded-lg bg-destructive/10 text-destructive text-sm text-center">
            {(updateProperty.error as { response?: { data?: { message?: string } } })
              ?.response?.data?.message || "Failed to update listing. Please try again."}
          </div>
        )}
      </form>
    </div>
